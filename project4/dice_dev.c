...
static ssize_t dice_read(struct file * file, char * buf, size_t count, loff_t *ppos) {
    char my_buffer[(count > 512) ? 512 : count];
    int i = 0;
    ...
    if( copy_to_user(buf, my_buffer, count) != 0) {
        printk(KERN_ERR "Error sending die rolls, read(file, 0x%p, %ld, %lld)\n", buf, (long int)count, *ppos);
        return -1;
    }
    *ppos = count;
    return count;
...
static ssize_t dice_write(struct file * file, const char * buf, size_t count, loff_t *ppos) {
    ...
    if(copy_from_user(my_buffer, buf, count) != 0) {
        printk(KERN_ERR "Error in copying from user, write(file, 0x%p, %ld, %lld)\n", buf, (long int)count, *ppos);
        return -1;
    }
    while(i < count) {
